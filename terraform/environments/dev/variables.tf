...
variable "allowed_ip_ranges" {
  description = "IP CIDR ranges to allow through Cloud Armor when access_mode is ip_restricted. Set in terraform.tfvars.local (gitignored) or via -var flag."
  type        = list(string)
  default     = []
}
...
variable "iap_access_group_email" {
  description = "Google Group email granted access through IAP when access_mode is iap"
  type        = string
  default     = ""
}
